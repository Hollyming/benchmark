...
def run_gharchive_pilot(
    ...
    project_dir = write_grounded_project(
        ...
        graph,
        probes,
        construction="gharchive_public_event_policy_pilot",
...
def build_gharchive_memory_graph(
    ...
    if not pr_events:
        raise ValueError("GHArchive pilot requires at least one pull request, review, or PR comment event")
    if not ci_events:
        raise ValueError("GHArchive pilot requires at least one CI/status/workflow event")
    if not negative_boundary_events:
        raise ValueError("GHArchive pilot requires a negative/boundary workflow event")
...
def _stage_plan_decision(profile: str, failed: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "blocking_checks": failed,
        "rationale": "Fixture profile is for engineering smoke tests only; do not use it for LLM/human annotation budget or paper claims.",
    }
